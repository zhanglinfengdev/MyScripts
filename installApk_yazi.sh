...
echo  $(adb install "$1") > ~/yyyyyyyyy.txt

echo "$1" >> ~/yyyyyyyyy.txt
...
if [ $? -eq 0 ]; then
    ...
    osascript << EOF
        display notification "$($1)" with title "APK 安装失败";
        tell application "Finder"' -e 'activate' -e 'display dialog "APK 安装失败 $($1)" with icon note' -e 'end tell
EOF
    exit 1
fi
