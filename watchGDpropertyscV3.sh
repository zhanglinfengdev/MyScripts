...
ADD_DEPENDENCY_SCRIPT="/Users/linfeng/scripts/MyScripts/watchGDadd_dependency.sh" # <<<--- CONFIRM THIS PATH
...
update_sketchybar() {
    ...
    while IFS= read -r file; do
      ...
      if [[ $? -ne 0 ]]; then log "  Error running awk on file '$file'."; continue; fi
...
if ! command -v jq &> /dev/null; then echo "Warning: jq not found. Popup cleanup might be less precise." >&2; log "jq command not found (warning issued)."; fi
# Check for git
if ! command -v git &> /dev/null; then echo "Error: git not found. Cannot reliably determine project root/branch." >&2; dependency_error=1; fi
# Check for the add_dependency script
if [[ ! -f "$ADD_DEPENDENCY_SCRIPT" ]]; then echo "Error: Dependency script not found at '$ADD_DEPENDENCY_SCRIPT'." >&2; dependency_error=1;
elif [[ ! -x "$ADD_DEPENDENCY_SCRIPT" ]]; then echo "Error: Dependency script '$ADD_DEPENDENCY_SCRIPT' is not executable (chmod +x)." >&2; dependency_error=1; fi

if [[ $dependency_error -eq 1 ]]; then log "Exiting due to missing critical dependencies or script issues."; exit 1; fi
