...
project_dirs["projectA"]="/Users/your_username/dev/projectA"
version_keys["projectA"]="VERSION" # Must match the key in projectA's gradle.properties
group_keys["projectA"]="GROUP"       # Must match the key in projectA's gradle.properties
artifact_id_keys["projectA"]="ARTIFACT_ID" # Must match the key in projectA's gradle.properties
gradle_configs["projectA"]="wrapper"
...
DEPENDENCY_OUTPUT_FILE="generated_dependencies.properties"
...
usage() {
  ...
  for name in "${!project_dirs[@]}"; do
    ...
    if [[ "$config_val" == "wrapper" ]]; then config_desc="Wrapper";
    elif [[ "$config_val" == "global" ]]; then config_desc="Global (PATH)";
    elif [[ "$config_val" == sdk:* ]]; then config_desc="Global (SDKMAN ${config_val#sdk:})";
    elif [[ "$config_val" == /* ]] || [[ "$config_val" == ~* ]]; then config_desc="指定路径";
...
if [ $# -eq 0 ]; then error_msg "脚本" "请至少提供一个配置名称。"; usage; fi
...
for config_name in "$@"; do
  ...
  if [[ ! -v project_dirs["$config_name"] ]]; then error_msg "$config_name" "未找到项目目录配置。"; overall_success=false; config_failed=true; continue; fi
  ...
  if [ ! -d "$PROJECT_DIR" ]; then error_msg "$config_name" "工程目录 '$PROJECT_DIR' 不存在。"; overall_success=false; config_failed=true; continue; fi
  ...
  case "$gradle_config_value" in
    wrapper)
      wrapper_path="$PROJECT_DIR/gradlew"; if [ ! -f "$wrapper_path" ]; then error_msg "$config_name" "'$wrapper_path' 未找到。"; overall_success=false; config_failed=true; continue; fi
      gradle_command="./gradlew"; gradle_command_source_info="Wrapper ($gradle_command)" ;;
    global)
      if command -v gradle &> /dev/null; then gradle_command="gradle"; gradle_command_path=$(command -v gradle); gradle_command_source_info="Global (PATH at $gradle_command_path)";
      else error_msg "$config_name" "'gradle' 命令未在 PATH 中找到。"; overall_success=false; config_failed=true; continue; fi ;;
    sdk:*)
      specific_version=${gradle_config_value#sdk:}; if [ -z "$specific_version" ]; then error_msg "$config_name" "'sdk:' 后面缺少版本号。"; overall_success=false; config_failed=true; continue; fi
      eval expanded_home_path="~"; sdk_path="$expanded_home_path/.sdkman/candidates/gradle/$specific_version/bin/gradle";
      if [ -f "$sdk_path" ] && [ -x "$sdk_path" ]; then gradle_command="$sdk_path"; gradle_command_source_info="Global (SDKMAN Version $specific_version)";
      else error_msg "$config_name" "SDKMAN Gradle '$specific_version' 未在 '$sdk_path' 找到。"; overall_success=false; config_failed=true; continue; fi ;;
    /* | ~*)
      eval expanded_gradle_path="$gradle_config_value";
      if [ -f "$expanded_gradle_path" ] && [ -x "$expanded_gradle_path" ]; then gradle_command="$expanded_gradle_path"; gradle_command_source_info="指定路径 ($expanded_gradle_path)";
      else error_msg "$config_name" "指定路径 '$expanded_gradle_path' 未找到或不可执行。"; overall_success=false; config_failed=true; continue; fi ;;
    *) error_msg "$config_name" "无效 Gradle 配置 '$gradle_config_value'。"; overall_success=false; config_failed=true; continue; ;;
  esac
  ...
  if [ ! -f "$PROPERTIES_FILE" ]; then error_msg "$config_name" "属性文件 '$PROPERTIES_FILE' 未找到。"; overall_success=false; config_failed=true; continue; fi
  ...
  if [ -z "$GROUP_LINE" ]; then error_msg "$config_name" "未找到键 '$group_key_prop'"; overall_success=false; config_failed=true; continue; fi
  if [ -z "$ARTIFACT_ID_LINE" ]; then error_msg "$config_name" "未找到键 '$artifact_id_key_prop'"; overall_success=false; config_failed=true; continue; fi
  if [ -z "$VERSION_LINE" ]; then error_msg "$config_name" "未找到键 '$version_key_prop'"; overall_success=false; config_failed=true; continue; fi
  ...
  if [ -z "$GROUP_VAL" ]; then error_msg "$config_name" "键 '$group_key_prop' 值为空"; overall_success=false; config_failed=true; continue; fi
  if [ -z "$ARTIFACT_ID_VAL" ]; then error_msg "$config_name" "键 '$artifact_id_key_prop' 值为空"; overall_success=false; config_failed=true; continue; fi
  if [ -z "$BASE_VERSION" ]; then error_msg "$config_name" "键 '$version_key_prop' 值为空"; overall_success=false; config_failed=true; continue; fi
  ...
  if [ $? -ne 0 ]; then error_msg "$config_name" "更新 '$PROPERTIES_FILE' 失败。"; overall_success=false; config_failed=true; continue; fi
  ...
  cd "$PROJECT_DIR"
  if [ $? -ne 0 ]; then error_msg "$config_name" "无法切换到目录 '$PROJECT_DIR'"; overall_success=false; config_failed=true; cd "$ORIGINAL_DIR" || echo "警告:无法切回"; continue; fi

  # If using wrapper, check executable permission now (Logic unchanged)
  if [[ "$gradle_config_value" == "wrapper" ]] && [[ ! -x "$gradle_command" ]]; then
      info_msg "$config_name" "警告: '$gradle_command' 不可执行。尝试添加权限..."; chmod +x "$gradle_command";
      if [ $? -ne 0 ]; then error_msg "$config_name" "无法为 '$gradle_command' 添加权限。"; overall_success=false; config_failed=true; cd "$ORIGINAL_DIR" || echo "警告:无法切回"; continue; fi
  fi

  # Execute uploadArchives
  info_msg "$config_name" "正在执行 '$gradle_command uploadArchives' 任务..."
  "$gradle_command" uploadArchives
  UPLOAD_EXIT_CODE=$?
  ...
  if [ $UPLOAD_EXIT_CODE -eq 0 ]; then
    ...
    echo "" >> "$DEPENDENCY_OUTPUT_FILE"
     ...
     overall_success=false; config_failed=true
