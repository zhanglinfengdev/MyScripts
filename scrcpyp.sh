...
scrcpy &
--max-size=1920 --bit-rate=1M --max-fps=15 --no-audio --turn-screen-off --no-show-touches --window-borderless --max-packets-in-flight=5 --disable-screensaver --encoder='OMX.google.h264.encoder'
...
# widDif=474+$(yabai -m query --windows --window | jq -r '{width: .frame.w, height: .frame.h}' | jq '.width')
