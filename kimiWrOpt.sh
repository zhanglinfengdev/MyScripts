...
kimiWrFile="/Users/didi/Documents/WeeklyReport/$prefix-kimiPolishing.md"

# 检查文件是否存在
if [ -f "$wrFile" -a -f "$kimiWrFile"  ]; then
  echo "文件 $kimiWrFile 已存在。"
else
  # 创建文件
  touch "$kimiWrFile"
  echo "文件 $kimiWrFile 已创建。"
fi
...
json_payload=$(cat <<EOF
{
    ...
    "messages": [
        ...
        {"role": "user", "content": "$payload <<< 对前面这段文本做周报内容优化,要求增强表达的专业性逻辑性和显示格式并对项目内容进行适当的描述展开并返回markdown格式的回复payload" }
    ],
    "temperature": 0.7
}
EOF
